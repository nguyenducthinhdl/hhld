...
type Verdict struct {
	OK     bool
	Reason string // code plus details, e.g. "stale_book venue=… gap_time=… max_book_age=…"

	// Info extras: stale_book {gap_time ms, venue}; negative_edge {net, buy_venue, sell_venue}.
	Info map[string]interface{}
}

// GapTimeMS returns Info["gap_time"] in milliseconds (stale_book), or 0 if unset.
func (v Verdict) GapTimeMS() int64 {
	return int64(v.FloatInfo("gap_time"))
}

// FloatInfo returns Info[key] as float64, or 0.
func (v Verdict) FloatInfo(key string) float64 {
	if v.Info == nil {
		return 0
	}
	switch n := v.Info[key].(type) {
	case float64:
		return n
	case float32:
		return float64(n)
...
// StringInfo returns Info[key] as string, or "".
func (v Verdict) StringInfo(key string) string {
	if v.Info == nil {
		return ""
	}
	s, _ := v.Info[key].(string)
	return s
}
